...
def _build_parser() -> argparse.ArgumentParser:
    ...
    p_print.add_argument(
        "--summary",
        help="Pre-generated 3-5 line narrative summary for the `receipt` template. "
        "Generated by the /receipt slash command in the parent Claude Code context; "
        "see ADR 0006. If absent, the receipt prints `(summary unavailable)`.",
    )
    p_print.add_argument(
        "--cost-override",
        type=float,
        help="Exact session cost in USD (e.g. from the statusline JSON). When "
        "given, overrides the pricing-table approximation; useful when the "
        "slash command can read Claude Code's runtime cost.",
    )
    p_print.add_argument(
        "--lines-added-override",
        type=int,
        help="Exact lines-added count from the runtime (overrides the structuredPatch tally).",
    )
    p_print.add_argument(
        "--lines-removed-override",
        type=int,
        help="Exact lines-removed count from the runtime (overrides the structuredPatch tally).",
    )
    return parser
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "print":
        ...
        ctx: dict[str, Any] = {
            ...
            "lines_added_override": args.lines_added_override,
            "lines_removed_override": args.lines_removed_override,
        }
        r = Receipt()
